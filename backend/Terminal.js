...
exports.safeTerminal = {
  installModules: async (backendPath, frontendPath) => {
    await Terminal(`cd ${backendPath}`);
    await Terminal(`npm install`)
    await Terminal(`cd ${frontendPath}`)
    await Terminal(`npm install`)
  },
  ...
  containerHistory: (container) => {
      return Terminal(`docker container logs ${container}`);
  },
  execMinecraftCommand: (task, id) => {
    const fullCommand = `docker exec ${id} rcon-cli ${task}`
    Log('Executing "' + fullCommand + '"', "Warning");
    return Terminal(fullCommand);
  }
};
